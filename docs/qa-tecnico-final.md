...
**Fecha**: 2026-07-11. **Estado del pipeline evaluado**: notebooks 01-05 completos, dashboard alpha en Tableau completo, hojas de vista longitudinal y transversal creadas (pendientes de integrar a un dashboard final), componente avanzado (PCA) implementado y pendiente de conectar en Tableau.
...
**Reproducibilidad**: los notebooks 03-05 solo dependen de la salida del notebook anterior (`outputs/week-0X-311/`), no del CSV crudo — se pueden re-ejecutar sin reprocesar 20.8M filas desde cero. Pendiente real (heredado de E4, ver `nyc-311-analytics-docs/seguimiento-proyecto.md`): no se ha corrido "Kernel → Restart & Run All" de la cadena completa 01→05 desde el CSV crudo en esta máquina, porque el archivo crudo (`data/311_Service_Requests_from_2020_to_Present_20260418.csv`, excluido del repo por tamaño) no está presente en este entorno. Los notebooks 04 y 05 sí se ejecutaron completos y sin errores a partir de los CSVs ya generados por los anteriores.
...
Regla aplicada consistentemente en todo el proyecto (definida en [E5-MEJORAS-DASHBOARD.md, paleta de 3 colores](../../nyc-311-analytics-docs/guias/E5-MEJORAS-DASHBOARD.md#paleta-de-3-colores)): **rojo o verde solo cuando hay una base de comparación clara** (un umbral, un promedio, un "mejor/peor"); azul neutro `#1F3A5C` para todo lo demás.

Verificación por gráfico (criterio del punto 5 de [E5-AJUSTES-DASHBOARD-ALPHA.md](../../nyc-311-analytics-docs/guias/E5-AJUSTES-DASHBOARD-ALPHA.md)):
...
Ya validado y cerrado en la ronda de ajustes de alpha — ver checklist completo en [E5-AJUSTES-DASHBOARD-ALPHA.md](../../nyc-311-analytics-docs/guias/E5-AJUSTES-DASHBOARD-ALPHA.md#checklist-de-esta-guía): una sola familia tipográfica en todo el workbook, escala de tamaños consistente (título 20-22px, KPI 28-32px, títulos de gráfico/KPI 12px, insights 11-12px), títulos de KPI sin jerga técnica, contenedores distribuidos en partes iguales.

**Pendiente para el dashboard final**: las hojas nuevas de E6 (`E6 - Tendencia temporal`, `E6 - Comparación por Borough`) todavía no se integraron a un dashboard — cuando se integren, deben seguir la misma escala de tamaños y la misma paleta de 3 colores que ya se cerró para alpha, no una paleta nueva.
...
| Paleta de 3 colores, rojo/verde solo con base de comparación | Evita "decorar" el dashboard con color sin significado | `nyc-311-analytics-docs/guias/E5-MEJORAS-DASHBOARD.md` |
...
| Criterio | Estado |
|---|---|
| El dashboard final responde la pregunta principal sin explicación externa extensa | 🟡 Alpha lo cumple; falta integrar las hojas de E6 (longitudinal, transversal, PCA) a un dashboard final |
| El pipeline técnico es reproducible y entendible | ✅ Notebooks 01-05, cada uno documentado y con sus propios checks |
| El equipo evidencia cobertura de todo el curso | 🟡 Falta cerrar historia visual y defensa (ver `nyc-311-analytics-docs/guias/E6-TRABAJO-FINAL.md`) |
| Se valida contraste, uso de color, etiquetas y títulos analíticos | ✅ Este documento, secciones 2-4 |
| Se incluye una vista longitudinal y una transversal claramente defendibles | 🟡 Hojas creadas (`E6 - Tendencia temporal`, `E6 - Comparación por Borough`); falta integrarlas a un dashboard e interpretarlas con insights escritos |
| La defensa muestra control de supuestos, límites y decisiones | ✅ Sección 5-6 de este documento + tabla de referencias en `E6-TRABAJO-FINAL.md`, sección 9 |
